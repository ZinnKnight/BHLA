...
func (s *Store) Save(ctx context.Context, sess Session, ttl time.Duration) error {
	...
	if err != nil {
		return fmt.Errorf("sessionvalidation: encode session: %w", err)
	}
	if err := s.rdb.Set(ctx, RedisKey(sess.SessionID), b, ttl).Err(); err != nil {
		return fmt.Errorf("sessionvalidation: save session: %w", err)
	}
	return nil
}

func (s *Store) Revoke(ctx context.Context, sessionID string) error {
	if err := s.rdb.Del(ctx, RedisKey(sessionID)).Err(); err != nil {
		return fmt.Errorf("sessionvalidation: revoke session: %w", err)
	}
	return nil
}
